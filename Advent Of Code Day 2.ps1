...
$part2 =@()
foreach($set in $list){
    $count, $letter, $password = $set -split " "
    [int]$min,[int]$max=$count -split "-"
    $letter = $letter -replace ":",""
    if($password -match $letter){
        [int]$charCount = ($password.ToCharArray() | Where-Object {$_ -eq "$letter"} | Measure-Object).count
        #write-host $charCount
        if(($charCount -ge $min) -and ($charCount -le $max)){            
            $part1 += $password
        }
        if(($password.ToCharArray()[$min-1] -eq $letter) -or ($password.ToCharArray()[$max-1] -eq $letter)){
            if(-not(($password.ToCharArray()[$min-1] -eq $letter) -and ($password.ToCharArray()[$max-1] -eq $letter))){
                $part2+=$password
            }            
        }
    } 
